...
command_output = subprocess.run(["netsh", "eth3", "show", "profiles"], capture_output = True).stdout.decode()
...
if len(profile_names) != 0:
    for name in profile_names:
        ...
        profile_info = subprocess.run(["netsh", "eth3", "show", "profile", name], capture_output = True).stdout.decode()
        ...
        if re.search("Security key           : Absent", profile_info):
            ...
            profile_info_pass = subprocess.run(["netsh", "eth3", "show", "profile", name, "key=clear"], capture_output = True).stdout.decode()
